...
class Api {
    ...
    public function getMaxExceeded()
    {
        return $this->maxSizeExceeded;
    }

	public function planInformation($apiKeys)
	{
		return $this->post(
            $this->planDetail,
            [
                'publicKey' => $apiKeys['publicTransferito_APIKey'],
                'secretKey' => $apiKeys['secretTransferito_APIKey'],
            ],
            true,
            true
        );
	...
	private function post($url, $body, $returnResultProperty = false, $overrideAPIKeys = false)
	{
		...
		if ($useFallback) {
            ...
            /**
             * Set the headers as a variable
             */
            $headers = $this->getRequestHeaders();

            /**
             * Override the API Keys
             * The body must have the array elements publicKey & secretKey
             */
            if ($overrideAPIKeys) {
                $headers['pKey'] = $body['publicKey'];
                $headers['sKey'] = $body['secretKey'];
            }
