...
class Config {
    ...
    public static function getChunkSize()
    {
        ...
        $uploadChunkSize = isset($settings['transferito_upload_chunk_size']) ? $settings['transferito_upload_chunk_size'] : false;

        if (!$uploadChunkSize) {
            return TRANSFERITO_CHUNK_SIZE;
        }

        $newUploadChunkSize = intval($uploadChunkSize);

        return ($newUploadChunkSize * 1024 * 1024);
    }
