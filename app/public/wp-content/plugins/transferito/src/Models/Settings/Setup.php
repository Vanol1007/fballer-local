...
class Setup {
    ...
    public function createOptions()
    {
        ...
        add_settings_field(
            'transferito_upload_chunk_size', // ID
            'Set upload chunk size', // Title
            array( $this, 'uploadChunkSizeField' ), // Callback
            'transferito-settings', // Page
            'transferito_settings_section'
        );
        ...
        add_settings_field(
            ...
            'Always upload to our secure servers', // Title
        ...
        add_settings_field(
            ...
            'Use TAR as default compression format', // Title
        ...
        add_settings_field(
            ...
            'Use default database configuration', // Title
        ...
        add_settings_field(
            ...
            'Create backup file without using exec', // Title
        ...
        add_settings_field(
            ...
            'Disable Malcare WAF plugin', // Title
    ...
    public function sanitize( $input )
    {
        ...
        if( isset( $input['transferito_upload_chunk_size'] ) ) {
            $new_input['transferito_upload_chunk_size'] = sanitize_text_field( $input['transferito_upload_chunk_size'] );
        }

        $new_input['transferito_force_upload'] = isset($input['transferito_force_upload']) && $input['transferito_force_upload'] == 'on' ? true : false;
        $new_input['transferito_include_htaccess'] = isset($input['transferito_include_htaccess']) && $input['transferito_include_htaccess'] == 'on' ? true : false;
        $new_input['transferito_force_tar_backup'] = isset($input['transferito_force_tar_backup']) && $input['transferito_force_tar_backup'] == 'on' ? true : false;
        $new_input['transferito_use_default_collation'] = isset($input['transferito_use_default_collation']) && $input['transferito_use_default_collation'] == 'on' ? true : false;
        $new_input['transferito_bypass_exec_archive_creation'] = isset($input['transferito_bypass_exec_archive_creation']) && $input['transferito_bypass_exec_archive_creation'] == 'on' ? true : false;
        $new_input['transferito_disable_wordpress_cache'] = isset($input['transferito_disable_wordpress_cache']) && $input['transferito_disable_wordpress_cache'] == 'on' ? true : false;
        $new_input['transferito_malcare_waf_plugin_fix'] = isset($input['transferito_malcare_waf_plugin_fix']) && $input['transferito_malcare_waf_plugin_fix'] == 'on' ? true : false;
    ...
    public function chunkSizeField()
    {
        ...
        $html .= $chunkSize == "" ? '<option selected disabled>Use default Download Chunk Size</option>' : '<option disabled>Use default Download Chunk Size</option>';
        ...
        $html .= $chunkSize == "100" ? '<option selected>100</option>' : '<option>100</option>';
        $html .= $chunkSize == "250" ? '<option selected>250</option>' : '<option>250</option>';
        $html .= '</select>';

        echo $html;
    }

    public function uploadChunkSizeField()
    {
        $chunkSize = isset( $this->options['transferito_upload_chunk_size'] ) ? esc_attr( $this->options['transferito_upload_chunk_size']) : '';

        $html = '<select id="transferito_upload_chunk_size" class="transferito-form-element transferito-input__dropdown transferito-input__dropdown--full-width transferito-input__dropdown--large" name="transferito_settings_option[transferito_upload_chunk_size]">';
        $html .= $chunkSize == "" ? '<option selected disabled>Use default Upload Chunk Size</option>' : '<option disabled>Use default Upload Chunk Size</option>';
        $html .= $chunkSize == "1" ? '<option selected>1</option>' : '<option>1</option>';
        $html .= $chunkSize == "5" ? '<option selected>5</option>' : '<option>5</option>';
        $html .= $chunkSize == "10" ? '<option selected>10</option>' : '<option>10</option>';
        $html .= $chunkSize == "25" ? '<option selected>25</option>' : '<option>25</option>';
