    ...
    $(document).ready(function(){
        ...
        function Transferito() {
            ...
            this.utilities = {
                ...
                saveTempMigrationDetails: function(details, key) {
                    this.tempMigrationDetails.details = details;
                    this.tempMigrationDetails.key = key;
                },
                clearTempMigrationDetails: function() {
                    this.tempMigrationDetails.details = {};
                    this.tempMigrationDetails.key = '';
                }
            };
            ...
            this.startMigration = function(wpNonce) {
                ...
                var sendFiles = $.post(ajaxurl, data, function(response) {
                    /**
                     * Show the completion screen if a local migration
                     *
                     * If not - Listen to the status updates
                     */
                    if (response.data.localMigration) {
                        self.utilities.hideHeaderLegend();
                        self.utilities.logEvent('localMigrationPartiallyCompleted', { completed:  true });
                        self.cleanUp(false, [], false, {}, null, true);
                    } else {
            ...
            this.prepareMigration = function (migrationDetails, key) {
                ...
                var backup = $.post(ajaxurl, data, function(response) {
                    ...
                    if (response.data.upgradeRequired) {
                        self.inPluginPurchase(migrationDetails, key);
                    }

                    /**
                     * If no premium is not required progress as normal
                     */
                    if (!response.data.upgradeRequired) {
                        /**
                         * Start the ZIP
             ...
             *
             * @param domain
             * @param localMigration
             * @param wpNonce
             * @param message
             * @param subMessage
             */
            this.checkCpanelAvailability = function (domain, localMigration, wpNonce, message, subMessage) {
                /**
                ...
                var cPanelCheck = $.post(ajaxurl, data, function(response) {
                    ...
                    var transferMethod = response.data.transferMethod;
                    var cPanelAllowed = response.data.cPanelAllowed;

                    self.utilities.setTransferMethodOptions(transferMethod, cPanelAllowed);
            ...
            /**
             * Check the API Keys Validity
             */
            this.apiValidityCheck = function(apiCheckPayload, loadingSelector) {
                /**
                 * Set the loading spinner
                 */
                this.utilities.setTemplate(
                    this.utilities.loadingScreenHTML(
                    'Please wait...',
                    'We are just validating your API Keys.'
                    ),
                    loadingSelector
                );
                 ...
                 * Show the element
                 */
                loadingSelector.removeClass('transferito__hide-element');

                var self = this;
                var data = {
                    action: 'check_premium_api_keys',
                    data: apiCheckPayload,
                    securityKey: apiCheckPayload.securityKey
                };

                /**
                 * The API Keys are valid
                 */
                var sendRequest = $.post(ajaxurl, data, function(response) {
                    /**
                     * Clear the loading Selector
                     */
                    self.utilities.setTemplate('', loadingSelector);

                    /**
                     * Display the Success Message
                     */
                    $('#upgradeToPremiumCheckComplete').removeClass('transferito__hide-element');

                    /**
                     * Log event
                     */
                    self.utilities.logEvent('upgradeToPremiumModalSuccess', {
                        upgraded: true
                    });
                });

                /**
                 * The API Keys aren't valid
                 */
                sendRequest.fail(function() {
                     ...
                     */
                    $('#upgradeToPremiumAPIKeyEntry').removeClass('transferito__hide-element');

                    /**
                     * Display the Error Message
                     */
                    $('#upgradeToPremiumErrorMessage').removeClass('transferito__hide-element');
                });

            }

            /**
             * Send user through the premium upgrade flow
             */
            this.inPluginPurchase = function(migrationDetails, key) {

                /**
                 * Log event
                 */
                transferito.utilities.logEvent('upgradeToPremiumModalFired', {
                    fired: true
                });

                /**
                 * Save the migration details
                 */
                transferito.utilities.saveTempMigrationDetails(migrationDetails, key);

                /**
                 * Fire the modal
                 */
                transferito.utilities.displayModal('upgradeToPremiumPaymentModal');
            }
        }
        ...
        /**
         * @todo InAPP Modal Upgrade - Event Listener
         */
        window.addEventListener('message', (event) => {
            if (event?.data?.type === 'transferitoNotifyParent') {

                /**
                 * Log event
                 */
                transferito.utilities.logEvent('upgradeToPremiumModalButtonClicked', {
                    fired: true
                });
         ...
         */
        transferito.utilities.selector.on('click', '#selectTransferitoMigrationMethod', function() {
        ...
        /**
         * When the Upgrade API Keys are pasted or changed
         * Enable or Disable the "ResumeMigration" button
         */
        transferito.utilities.modalSelector.on('paste change keyup', '.transferito-input__upgrade-premium-api-keys', function(event) {
            setTimeout(function(){
                var validateFields = transferito.utilities.validateFormFields();

                /**
                 * If the fields are validated
                 * The "validateFormFields" method returns true
                 * Flip the response
                 */
                $('#updateYourAPIKeys').prop('disabled', !validateFields);
            },0);
        });
         ...
         * Process sending the guide details
         */
        transferito.utilities.modalSelector.on('click', '.transferito_upgrade-save-api-keys', function() {
            /**
             * Disable the Button to stop double checks
             */
            $(this).prop('disabled', true);
            ...
            if (validateFields) {
                ...
                var loadingSelector = $('#upgradeToPremiumLoading');
                var apiKeyPayload = {
                    publicKey: $('#upgradePremiumPublicKey').val(),
                    secretKey: $('#upgradePremiumSecretKey').val(),
                    securityKey: $('#upgradePremiumValidation').val()
                };

                /**
                 * Hide API Entry Screen
                 */
                $('#upgradeToPremiumAPIKeyEntry').addClass('transferito__hide-element');

                /**
                 * Run the API Key Check
                 */
                transferito.apiValidityCheck(apiKeyPayload, loadingSelector);
            }
         ...
         * Process sending the guide details
         */
        transferito.utilities.modalSelector.on('click', '.transferito_upgraded-in-app-resume-migration', function() {
            /**
             * Disable the Button to stop double migrations being fired
             */
            $(this).prop('disabled', true);

            /**
             * Fire the migration with the previous details
             */
            transferito.prepareMigration(
                transferito.utilities.tempMigrationDetails.details,
                transferito.utilities.tempMigrationDetails.key
            );

            /**
             * Reset the temp migration details
             */
            transferito.utilities.clearTempMigrationDetails();
        });
