	...
	<div class="container">
		<div class="search-inner">
			<div class="container ajax-posts" ajax-data="<?php echo $ajaxVariable; ?>">
				...
				<?php if ( $value = get_theme_mod('custom_content_archive_champs') ) { ?>
				...
				<div class="search-filter__row">
					<!-- <button class="apply-filter" data-target="filter-3">
					...
					<div class="filter-chiose__items active" id="filter-3">
						<div class="input-wrap filter-chiose__item lg">
							<label for="date">Дата</label>
							<input type="date" id="date" value="<?php //echo wp_date('Y-m-d', time()); 
																									?>" />
						</div>
						...
						<?php if (! is_wp_error($terms) && ! empty($terms)) { ?>
							...
							<div class="input-wrap filter-chiose__item lg">
								<label for="game_format">Формат</label>
						...
						$terms = get_terms(array(
							'taxonomy' => 'all',
							'fields' => 'champ_price', // ids, names, count, id=>parent
							'hide_empty' => false,
						));
						?>
						...
						<?php if (! is_wp_error($terms) && ! empty($terms)) { ?>
							...
							<div class="input-wrap filter-chiose__item lg">
								<label for="champ_price">Цена за игру</label>
								<select name="champ_price" id="champ_price">
									<option value="" selected>Все</option>
						...
						<div class="input-wrap filter-chiose__item">
							<button class="primary-btn filter-btn">Поиск</button>
						</div>
					</div>
				</div>
