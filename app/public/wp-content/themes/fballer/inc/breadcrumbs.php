...
// Оригинал - http://dimox.name/wordpress-breadcrumbs-without-a-plugin/
function theme_breadcrumbs() {
	...
	$text['404']      = 'Ошибка 404';
	$text['page']     = 'Страница %s'; // Пагинация
	$text['cpage']    = 'Страница комментариев %s'; // Страница комментариев
	$wrap_before    = '<div class="breadcrumbs" itemscope itemtype="http://schema.org/BreadcrumbList">';
	$wrap_after     = '</div><!-- .breadcrumbs -->';
	$sep            = '<span class="breadcrumbs__separator"> / </span>'; // Разделитель
    ...
    if ( $show_home_link ) {
        if ( $position > 1 ) echo $sep;
        echo $home_link; // Выводим "Главная"
        $position += 1;
    }
    if ( $show_current ) {
        if ( $position >= 1 ) echo $sep;
        echo $before . 'Поиск полей' . $after; // Выводим текущий элемент
