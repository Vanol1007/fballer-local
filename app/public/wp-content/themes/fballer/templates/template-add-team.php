<?php
/*
Template Name: Поиск игрока
*/

$result = fballer_handle_frontend_submission('teams');
...
$matched_player_ids = ( $success && $created_post_id > 0 ) ? fballer_get_matching_players_for_team($created_post_id, 6) : [];
?>
...
<section class="section place-detail">
	...
	<div class="container">
		...
		<div class="search-inner">
			<div class="section-title">Команда ищет игрока</div>
			...
			<?php if ( $success ) { ?>
				...
				<?php if ( ! empty($matched_player_ids) ) { ?>
					<div class="players">
						<div class="section-title">Эти игроки могли бы вам подойти</div>
						<div class="player-items">
							<?php
							$matched_query = new WP_Query([
								'post_type' => 'players',
								'post__in' => $matched_player_ids,
								'orderby' => 'post__in',
								'posts_per_page' => count($matched_player_ids),
							]);
			...
			<form class="form add-form" method="post" action="">
				...
				<?php wp_nonce_field('fb_add_teams', '_fb_nonce'); ?>
				...
				<div class="input-row">
					<div class="input-wrap">
						<label for="team_btn_link">Ссылка на Telegram</label>
						<input type="url" id="team_btn_link" name="team_btn_link" placeholder="https://t.me/username" value="<?php echo esc_attr($_POST['team_btn_link'] ?? ''); ?>">
						<input type="hidden" name="team_btn_text" value="<?php echo esc_attr($_POST['team_btn_text'] ?? 'Написать'); ?>">
					</div>
					<div class="input-wrap">
				...
				<div class="input-row input-row--message">
					<div class="input-wrap input-wrap--textarea">
						...
						<textarea id="content" name="content" placeholder="Ищем игрока в команду. Тренировки и игры в вашем городе. Напишите нам в Telegram."><?php echo esc_textarea($_POST['content'] ?? ''); ?></textarea>
					</div>
