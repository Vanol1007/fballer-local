<?php
/*
Template Name: Чемпионаты
*/

$ajaxVariable = 'ajaxChampsData';
...
if ( is_array($game_formats) ) {
	...
	usort($game_formats, static function( $left, $right ) {
		preg_match('/^(\d+)/', (string) $left->name, $left_matches);
		preg_match('/^(\d+)/', (string) $right->name, $right_matches);

		$left_number = isset($left_matches[1]) ? (int) $left_matches[1] : 999;
		$right_number = isset($right_matches[1]) ? (int) $right_matches[1] : 999;
		...
		<div class="search-inner">
			<div class="container ajax-posts" ajax-data="<?php echo $ajaxVariable; ?>">
				...
				<div class="search-filter__row search-filter__row--games">
					...
					$desktop_filter_columns = 1 + $visible_geo_filters_count;
					?>
				...
				<div class="championship-items search-champ_items ajax-posts__items">
...
document.addEventListener('DOMContentLoaded', function () {
  var root = document.querySelector('.page-template-template-champs .games-filter-form');
  if (!root) return;
