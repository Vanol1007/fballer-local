...
if ( is_array($game_formats) ) {
	usort($game_formats, static function( $left, $right ) {
		...
		$left_size = preg_match('/(\d+)\D+(\d+)/u', $left_name, $left_matches)
			? array((int) $left_matches[1], (int) $left_matches[2])
			: array(PHP_INT_MAX, PHP_INT_MAX);
		$right_size = preg_match('/(\d+)\D+(\d+)/u', $right_name, $right_matches)
			? array((int) $right_matches[1], (int) $right_matches[2])
			: array(PHP_INT_MAX, PHP_INT_MAX);
		...
		if ( $left_size[1] !== $right_size[1] ) {
			return $left_size[1] <=> $right_size[1];
		}
		...
		<div class="search-inner">
			<div class="container ajax-posts" ajax-data="<?php echo $ajaxVariable; ?>">
				...
				<div class="search-filter__row search-filter__row--games">
					...
					$desktop_filter_columns = 2 + $visible_geo_filters_count;
					?>
							...
							<div class="input-row games-filter-form__row" style="--games-filter-columns: <?php echo (int) $desktop_filter_columns; ?>;">
								<div class="input-wrap">
									...
									<div class="multi-select search-multi-select" data-placeholder="Все">
										...
										<div class="multi-select__panel" role="listbox">
											<?php foreach ( (array) $game_formats as $term ) { ?>
								...
								<div class="input-wrap">
									...
									<div class="multi-select search-multi-select" data-placeholder="Все">
										<button type="button" class="multi-select__btn" aria-haspopup="listbox" aria-expanded="false">Все</button>
										<div class="multi-select__panel" role="listbox">
											<?php foreach ( (array) $coating_terms as $term ) { ?>
												<label class="multi-select__item">
													<input type="checkbox" name="coating[]" value="<?php echo (int) $term->term_id; ?>">
													<span><?php echo esc_html($term->name); ?></span>
												</label>
											<?php } ?>
										</div>
...
document.addEventListener('DOMContentLoaded', function () {
  var root = document.querySelector('.page-template-template-places .games-filter-form');
  if (!root) return;
